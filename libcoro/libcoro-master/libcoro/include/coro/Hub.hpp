...
// Cleanup Hub for current thread (call before thread exits)
void cleanupHub();
...
public:
    ...
    template <typename F>
    Ptr<Coroutine> start(F func, uint32_t stackSize) {
        Ptr<Coroutine> coro(new Coroutine(func, stackSize));
        runnable_.push_back(coro);
        return coro;
    }
    void quiesce();
...
template <typename F>
Ptr<Coroutine> start(F func, uint32_t stackSize) {
    return hub()->start(func, stackSize);
}
