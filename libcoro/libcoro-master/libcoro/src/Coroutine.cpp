...
extern "C" {
// 线程局部存储，每个线程有自己的当前协程指针
thread_local coro::Coroutine* coroCurrent = nullptr;
}

// 初始化当前线程的 coroCurrent
static void initCoroCurrent() {
    if (!coroCurrent) {
        coroCurrent = coro::main().get();
    }
}
...
bool Coroutine::join(const Time& timeout) {
    assert(current().get() != this);
    Time end = Time::now() + timeout;
    while (status_ != Coroutine::EXITED) {
        Time remaining = end - Time::now();
        if (remaining <= Time::sec(0)) {
            return false;
        }
        event_->timedWaitFor(remaining);
    }
    return true;
}
...
Ptr<Coroutine> main() {
    // Thread-local storage: each thread has its own main coroutine
    thread_local Ptr<Coroutine> mainCoro;
    if (!mainCoro) {
        mainCoro.reset(new Coroutine);
        registerSignalHandlers();
        // Initialize coroCurrent pointer for current thread
        coroCurrent = mainCoro.get();
    }
    return mainCoro;
}
